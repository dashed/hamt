...
*/
const updateCollisionList = (h, list, f, k, defaultValue, sizable) => {
    let target;
    let i = 0;
    ...
    return target && v === target.value ? list :
        v === nothing ? arraySpliceOut(i, list)
...
Leaf.prototype._modify = function(shift, f, h, k, defaultValue, sizable) {
   if (k === this.key) {
        ...
        return v === this.value ? this :
               v === nothing ? empty : new Leaf(h, k, v);
    }
...
Collision.prototype._modify = function(shift, f, h, k, defaultValue, sizable) {
    if (h === this.hash) {
        const list = updateCollisionList(this.hash, this.children, f, k, defaultValue, sizable);
        return list === this.children ? this :
            list.length > 1
...
IndexedNode.prototype._modify = function(shift, f, h, k, defaultValue, sizable) {
    ...
    const child = current._modify(shift + SIZE, f, h, k, defaultValue, sizable);
...
ArrayNode.prototype._modify = function(shift, f, h, k, defaultValue, sizable) {
    ...
    const newChild = (child || empty)._modify(shift + SIZE, f, h, k, defaultValue, sizable);
...
empty._modify = (_, f, h, k, defaultValue, sizable) => {
    const v = f(defaultValue);
...
const modifyHash = hamt.modifyHash = (f, hash, key, map, defaultValue) => {
    ...
    const currentSize = map._size;
    const sizable = {s: currentSize};

    const newRoot = map.root._modify(0, f, hash, key, defaultValue, sizable);

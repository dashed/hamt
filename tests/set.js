...
describe('set', () => {
    ...
    it('does not return new map when new value is the same as old value', () => {
        ...
        assert.strictEqual(h, h2 = h2.set(FOO, 'value'));
        assert.strictEqual(h, h2 = h2.set(BAR, BAZ));
        assert.strictEqual(h, h2 = h2.set('falsy', null));
        assert.strictEqual(false, h === h2.set(BAR, { prop: 1}));
